...
app.put("/localdb/:id", (req, res) => {
  const { id } = req.params;
  const localData = readData();
  const foundPostIndex = localData.findIndex(
    (post) => post.id === parseInt(id)
  );
  if (foundPostIndex !== -1) {
    localData[foundPostIndex] = req.body;
    writeData(localData);
    res.json(req.body);
  } else {
    res.status(404).json({ error: "Post not found" });
...
app.delete("/localdb/:id", (req, res) => {
  ...
  if (foundPostIndex !== -1) {
    localData.splice(foundPostIndex, 1);
    writeData(localData);
    res.status(204).send();
  } else {
    res.status(404).json({ error: "Post not found" });
